; GPIO_PORTE and ADC0 address, SS3
; PE 5 (AIN 8)
...
; activate clock for ADC0
		...
		LDR R1, =ADC0_ACTSS_R       
		LDR R0, [R1]           
		BIC R0, R0, #0x0F				; disable SS3 before configuration to 
		STR R0, [R1]    				; prevent erroneous execution if a trigger event were to occur

		LDR R1, =ADC0_EMUX_R       
		LDR R0, [R1]           
		BIC R0, R0, #0x000F				; SS3 is software trigger
		STR R0, [R1]    
		...
		LDR R1, =ADC0_IM_R     
		LDR R0, [R1]           
		BIC R0, R0, #0x08				; disable SS3 interrupts
		STR R0, [R1]    
		
		LDR R1, =ADC0_ACTSS_R      
		LDR R0, [R1]           
		ORR R0, R0, #0x08     		; enable SS3
		STR R0, [R1]
...
Loop	
		...
		LDR R1, =ADC0_ISC_R
		LDR R0, [R1]
		ORR R0, R0, #0x08					; acknowledge conversion
		STR R0, [R1]
