; GPIO_PORTE and ADC0 address, SS0
; PE 5 (AIN 8)
...
; activate clock for ADC0
		...
		LDR R1, =ADC0_SSPRI_R       
		LDR R0, =0x1230           		; SS0 is highest priority
		STR R0, [R1]    

		LDR R1, =ADC0_ACTSS_R       
		LDR R0, [R1]           
		BIC R0, R0, #0x0F				; disable SS0 before configuration to 
		STR R0, [R1]    				; prevent erroneous execution if a trigger event were to occur

		LDR R1, =ADC0_EMUX_R       
		LDR R0, [R1]           
		BIC R0, R0, #0x000F				; SS0 is software trigger
		STR R0, [R1]    

		LDR R1, =ADC0_SSMUX0_R      
		LDR R0, [R1]           
		BIC R0, R0, #0x000F				; clear SS0 field
		ADD R0, R0, #8					; set channel -> select input pin AIN11
		STR R0, [R1]    

		LDR R1, =ADC0_SSCTL0_R       
		...
		LDR R1, =ADC0_IM_R     
		LDR R0, [R1]           
		BIC R0, R0, #0x01				; disable SS0 interrupts
		STR R0, [R1]    
		
		LDR R1, =ADC0_ACTSS_R      
		LDR R0, [R1]           
		ORR R0, R0, #0x01     		; enable SS0
		STR R0, [R1]
...
Loop	
		LDR R1, =ADC0_PSSI_R      
		MOV R0, #0x01					; initiate sampling in the SS0  
		STR R0, [R1]    
		
		LDR R1, =ADC0_RIS_R   			; R1 = address of ADC Raw Interrupt Status
		LDR R0, [R1]           			; check end of a conversion
		CMP	R0, #0x01    				; when a sample has completed conversion -> a raw interrupt is enabled
		BNE	Loop    
		
		LDR R1, =ADC0_SSFIFO0_R			; load SS3 result FIFO into R1
		LDR R0,[R1]
		...
		LDR R1, =ADC0_ISC_R
		LDR R0, [R1]
		ORR R0, R0, #01					; acknowledge conversion
		STR R0, [R1]
